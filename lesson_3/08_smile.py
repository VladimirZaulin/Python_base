...
def square_smile(x, y, color):
    ...
    head = rectangle(base_point, get_point(x + 100, y + 100), color, 3)
    x += 75
    y += 65
    right_eye = circle(get_point(x, y), 10, color, 3)
    x -= 50
    left_eye = circle(get_point(x, y), 10, color, 3)
    y -= 40
    mouth = ellipse(get_point(x, y), get_point(x + 50, y + 7), color, 6)
